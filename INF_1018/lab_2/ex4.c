...
void dump (void *p, int n) {//endereço de i (=10000) e n = 4 (sizeof(int))
  ...
  int main (void) {
    char caractereInt = 97; //Incializa e Declara i = 10000
    dump(&caractereInt, sizeof(caractereInt));//Chama a função Dump
    
    char caractereChar = 'a';
    dump(&caractereChar, sizeof(caractereChar));
    return 0;
  }


//Tamanho em bytes de um long é = 2 bytes
//Estão ordenados no caso do meu PC em little endian então o menos significativo é impresso primeiro.


//No caso do show acontece a mesma coisa, porémm são mais bytes (2 bytes)
